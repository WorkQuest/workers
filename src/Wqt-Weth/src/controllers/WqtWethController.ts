...
export class WqtWethController implements IController {
  ...
  protected async syncEventHandler(eventsData: EventData) {
    ...
    this.Logger.debug('Sync event handler: (tx hash "%s") tokens pool (shifted by -18): eth "%s", wqt "%s"',
      transactionHash,
      ethPool,
      wqtPool,
    );
    ...
    if (!tokenETHPriceInUsd || !tokenWQTPriceInUsd) {
      this.Logger.warn('Sync event handler: (tx hash "%s") tokens price (eth "%s", wqt "%s") in usd at timestamp "%s" moment is not found',
        transactionHash,
        tokenETHPriceInUsd,
        tokenWQTPriceInUsd,
        timestamp,
    ...
    this.Logger.debug('Sync event handler: (tx hash "%s") tokens price in usd: eth "%s", wqt "%s"',
      transactionHash,
      tokenETHPriceInUsd,
      tokenWQTPriceInUsd,
    );
